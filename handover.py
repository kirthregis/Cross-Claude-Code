...
ledger = '''# EMY STUDIO - HANDOVER PACKAGE
...
- AED IBAN: ***REDACTED-IBAN***
- GBP IBAN: ***REDACTED-IBAN***
- USD IBAN: ***REDACTED-IBAN***
- EUR IBAN: ***REDACTED-IBAN***
...
## CREDENTIALS
- Admin Password: ***REDACTED-ADMIN-TOKEN***
...
with open("HANDOVER.md", "w", encoding="utf-8") as f:
    f.write(ledger)
print(f"WROTE HANDOVER.md ({os.path.getsize('HANDOVER.md')} bytes)")

# Update STATUS_LEDGER.md
status = '''# EMY STUDIO - STATUS LEDGER
Last Updated: August 2026
...
with open("STATUS_LEDGER.md", "w", encoding="utf-8") as f:
    f.write(status)
print(f"WROTE STATUS_LEDGER.md ({os.path.getsize('STATUS_LEDGER.md')} bytes)")

subprocess.run(["git", "add", "-A"], check=True)
r = subprocess.run(["git", "status", "--short"], capture_output=True, text=True)
print(r.stdout)
subprocess.run(["git", "commit", "-m", "Handover package: full ledger with known issues, fixes needed, clone instructions"], check=True)
