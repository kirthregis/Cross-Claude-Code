...
function paintAlerts(){
  ...
  var api=API();
  var link='<a href="#" onclick="setApi();return false" style="color:#ef4444">'
    + (api ? 'Server: '+api.replace(/^https?:\\/\\//,'') : 'Connect server for background alerts')
    + '</a>';
  var state='';
  if(alertsOn()) state = api
    ? 'On \u2014 gigs ping this phone even with the app closed.'
    : 'On \u2014 pings while the app is open. Connect the server for alerts when it is closed.';
  st.innerHTML = state + '<br>' + link;
}

function toggleAlerts(){
  if(alertsOn()){
    localStorage.setItem(ALERTS_KEY,'0');
    if(API()&&navigator.serviceWorker) navigator.serviceWorker.ready.then(function(r){
  ...
  Notification.requestPermission().then(function(p){
    if(p!=='granted'){ paintAlerts(); return; }
...
/* Real Web Push — arrives with the app CLOSED. Needs the Worker. */
function subscribePush(){
  var api=API();
  if(!api||!('serviceWorker' in navigator)||!('PushManager' in window)){
    notify('Alerts on','New gigs ping you when the app is open.');
    return;
  }
  fetch(api+'/vapid').then(function(r){return r.json()}).then(function(d){
    if(!d.key) throw 0;
    return navigator.serviceWorker.ready.then(function(reg){
      return reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlB64(d.key)});
    });
  }).then(function(sub){
    return fetch(api+'/subscribe',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify(sub)});
  }).then(function(){ paintAlerts(); })
  .catch(function(){
    notify('Alerts on','New gigs ping you when the app is open.');
    paintAlerts();
  });
...
function checkFeed(){
  if(!alertsOn()) return;
  fetch((API()? API()+'/feed.json' : 'feed.json')+'?t='+Date.now()).then(function(r){return r.ok?r.json():null}).then(function(d){
    if(!d||!d.gigs) return;
    var seen={};
