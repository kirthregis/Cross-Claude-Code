      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3 gap-2">
        ...
        <nav className="flex items-center gap-1 overflow-x-auto min-w-0">
          {tabs.map((t) => {
            ...
            return (
              <Link key={t.href} href={t.href} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition whitespace-nowrap ${active ? "bg-zinc-800 text-white" : "text-zinc-400 hover:text-white"}`}>
                {t.label}
              </Link>
            );
          })}
