...
import { createProject, deleteProject, useProjects, useSettings, saveSettings } from "@/lib/studio/store";
...
export default function StudioHome() {
  ...
  const [kind, setKind] = useState<ProjectKind>("mix");
  ...
  const goToTab = (tab: string) => {
    if (projects.length > 0) {
      router.push("/studio/p/" + projects[0].meta.id + "?tab=" + tab);
    } else {
      setShowForm(true);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="mt-1 max-w-xl text-sm text-zinc-400">Master the mix, design the cover, package the release, pass the platform checks.</p>
        </div>
      ...
      {showForm && (
        <Card className="p-4 sm:p-5">
          ...
          <form className="mt-3 grid gap-3 sm:grid-cols-2" onSubmit={e => { e.preventDefault(); if (!name.trim()) return; makeProject(name.trim()); }}>
            ...
            <input value={mood} onChange={e => setMood(e.target.value)} placeholder="Mood for the cover"
              className="rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-fuchsia-500 focus:outline-none sm:col-span-2" />
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {[
          { icon: "??", title: "Master in-browser", text: "EQ, compression, limiting and loudness to -14 LUFS.", tab: "master" },
          { icon: "??", title: "Cover art in seconds", text: "AI-designed covers exported at 3000x3000.", tab: "artwork" },
          { icon: "??", title: "Release-ready", text: "Title, description, tags, checks.", tab: "release" },
        ].map(f => (
          <button key={f.title} onClick={() => goToTab(f.tab)}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Link href="/studio/gigradar" className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-900/70 to-zinc-900/30 px-4 py-3.5 transition hover:border-fuchsia-500/50">
          <div className="flex items-center gap-3">
            ...
            <div><div className="text-sm font-bold text-zinc-100">GigRadar</div><div className="text-xs text-zinc-500">Find gigs, generate AI pitch emails.</div></div>
          </div>
        ...
        <Link href="/studio/analytics" className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-900/70 to-zinc-900/30 px-4 py-3.5 transition hover:border-fuchsia-500/50">
          <div className="flex items-center gap-3">
            ...
            <div><div class="text-sm font-bold text-zinc-100">Analytics</div><div class="text-xs text-zinc-500">Live YouTube stats and performance.</div></div>
          </div>
          <span className="brand-text shrink-0">Open ?</span>
        </Link>
        <Link href="/studio/distribute" className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-900/70 to-zinc-900/30 px-4 py-3.5 transition hover:border-fuchsia-500/50">
          <div className="flex items-center gap-3">
            <span className="text-2xl">??</span>
            <div><div className="text-sm font-bold text-zinc-100">Distribute</div><div className="text-xs text-zinc-500">Smart links, royalty splits.</div></div>
          </div>
          <span className="brand-text shrink-0">Open ?</span>
        </Link>
        <Link href="/studio/community" className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-900/70 to-zinc-900/30 px-4 py-3.5 transition hover:border-fuchsia-500/50">
          <div className="flex items-center gap-3">
            <span className="text-2xl">??</span>
            <div><div className="text-sm font-bold text-zinc-100">MENA Community</div><div className="text-xs text-zinc-500">Connect with artists across the region.</div></div>
          </div>
          <span className="brand-text shrink-0">Open ?</span>
        </Link>
      </div>
